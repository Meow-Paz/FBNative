...
void *handleClient(void *args) {
	...
	list_add(l, n);
	pthread_t onconthr;
	pthread_create(&onconthr,NULL,(void *(*)(void*))onConnection,(void*)n);
	pthread_setcancelstate(PTHREAD_CANCEL_ENABLE, NULL);
	...
	while (1) {
		...
		pthread_setcancelstate(PTHREAD_CANCEL_DISABLE, NULL);
		char *realmsg=malloc(strlen(n->message->msg)+1);
		memcpy(realmsg,n->message->msg,strlen(n->message->msg)+1);
		realmsg[strlen(n->message->msg)]=0;
...
int main(int argc, char *argv[]) {
	...
	printf("Server is running at ws://127.0.0.1:");
